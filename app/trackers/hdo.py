...
class HDOTracker(Tracker):
    ...
    def get_info(self) -> dict:
        return {'min_ratio': 0.4, 'min_seedtime': '72hours'}

    def get_data(self) -> dict:
        url = f"https://hd-olimpo.club/users/{self.username}"
        response = requests.get(url, cookies=self.cookies)
        if response.status_code != 200:
            raise ValueError(f"Failed to fetch data from {self.name} tracker. Status code: {response.status_code}")
        soup = BeautifulSoup(response.text, 'html.parser')
        user_class = soup.findAll('span', class_='badge-user')[1].text.strip()
        ...
        for item in items:
            ...
            if key in ('Descarga guardada', 'Subida guardada'):
                value = value.split(' ')[0]
            data[key] = value
